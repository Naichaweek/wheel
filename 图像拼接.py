...
def remove_seam(image1, image2, mask):
    # 将掩码转换为uint8类型
    mask = mask.astype(np.uint8)
    # 拼接图像的修复
    result = cv2.inpaint(image1, mask, 3, cv2.INPAINT_TELEA)

    return result



img1 = cv2.imread("testv/v1/css-R-LL.jpg")
img2 = cv2.imread("testv/v1/css-R-LM.jpg")
# 转换为灰度图像
img3 = cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY)
img4 = cv2.cvtColor(img2, cv2.COLOR_BGR2GRAY)

sift = cv2.SIFT_create(nfeatures=512, nOctaveLayers=3, contrastThreshold=0.0025, edgeThreshold=0.3, sigma=1.1)
...
img1 = cv2.imread("testv/v1/css-R-LL.jpg",cv2.IMREAD_UNCHANGED)
img2 = cv2.imread("testv/v1/css-R-LM.jpg",cv2.IMREAD_UNCHANGED)

# 对img1图片进行变换
warpImg = cv2.warpPerspective(img1, M, (img1.shape[1], img1.shape[0]))


mask = cv2.cvtColor(warpImg, cv2.COLOR_BGR2GRAY)
# 调用函数进行融合
...
# overlapping = cv2.max(warpImg, img2)
# #会变亮
# # overlapping = cv2.addWeighted(img2, 1, result,1, 0., result, -1)
# cv2.imwrite("LM_LL.png",overlapping)
